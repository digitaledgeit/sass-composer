...
module.exports.transforms = {
  ...
  hashed: function(options) {
    ...
    return function(filename, url, done) {
      ...
      //compute the hashed path
      var
        src     = path.resolve(fdir, url),
        hashed  = path.relative(rdir, src).replace(/\\/g, '/')
      ;

      //compute a hash of the file contents to use for the destination file name
      hashFile(src, function(err, contentsHash) {
        var rel   = contentsHash+path.extname(src);
        ...
        if (copy) {
          ...
          var
            parser = require('url'),
            srcFile = path.resolve(fdir,
            parser.parse(url).pathname),
