  ...
  relative: function(options) {
    ...
    return function(filename, url, done) {
      ...
      if (copy) {
        ...
        //FIXME: url.parse doesn't work with window drive names
        var url = require('url');
        var parts = url.parse(src);
        src = parts.pathname;

        //compute the destination URL
        var dest = path.resolve(dir, rel);

        //don't overwrite the file with itself cause that'll break it
        if (src === dest) {
          debug('exists:    "%s" in "%s" to "%s"', url, path.relative(rdir, filename), rel);
          return done(null, rel);
        }
