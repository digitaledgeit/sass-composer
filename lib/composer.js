...
//the number of times .compose() has been called
var run = 0;
...
Composer.prototype.compose = function(callback) {
  ...
  function end(err, css, stats) {
    process.nextTick(function() {
      ...
      if (err) {
        ...
        //stream.end(null); //normal streams don't end when there's an error
      } else {
        stream.end(css);
      }
  ...
  this.resolve(
    ...
    function(err, ctx) {
      if (err) return end(err, null, null);
